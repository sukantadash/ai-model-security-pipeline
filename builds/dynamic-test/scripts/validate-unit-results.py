#!/usr/bin/env python3
"""Validate dynamic-scan unit-test JSON against the expected finding contract."""
...
CHECKS = {
    "dynamic-isolated-runtime.json": {
        ...
        "min_findings": 1,
        "issue_contains": "not a Kata/QEMU guest",
        "risk": "critical",
    ...
    "dynamic-behavior.json": {
        "subtask": "behavior",
        "min_findings": 1,
        "issue_contains": "model_sandbox_shell",
        "risk": "critical",
        "tool_used": "falco",
    },
    ...
    "dynamic-basic-inference.json": {
        "subtask": "basic-inference",
        "min_findings": 1,
        "issue_contains": "no loadable weight files",
        "risk": "critical",
        "tool_used": "vllm",
    },
}
...
def main() -> int:
    ...
    for filename, spec in CHECKS.items():
        ...
        for row in findings:
            ...
            if missing:
                ...
                continue
            if row.get("task") != "dynamic-scan":
                errors.append(f"{filename}: task={row.get('task')!r} want dynamic-scan")
    ...
    if errors:
        ...
        print("\n".join(errors))
        return 1
    print("PASS: dynamic-scan unit findings match expected schema and issues")
    return 0
